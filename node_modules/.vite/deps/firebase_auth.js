import { A as getDefaultEmulatorHost, D as createSubscribe, F as isBrowserExtension, I as isEmpty, L as isIE, M as getExperimentalSetting, N as getModularInstance, O as deepEqual, P as getUA, S as FirebaseError, U as querystring, V as isReactNative, W as querystringDecode, b as Component, c as _getProvider, f as getApp, g as registerVersion, k as extractQuerystring, l as _registerComponent, n as SDK_VERSION, v as LogLevel, w as base64Decode, x as ErrorFactory, y as Logger, z as isMobileCordova } from "./index.esm2017-BW30Pwdb.js";
...
//#endregion
//#region node_modules/@firebase/auth/dist/esm2017/index-e3d5d3f4.js
/**
* @license
...
async function injectRecaptchaFields(auth, request, action, captchaResp = false) {
	...
	if (!captchaResp) Object.assign(newRequest, { captchaResponse });
...
var AuthImpl = class {
	...
	_initializeWithPersistence(persistenceHierarchy, popupRedirectResolver) {
		...
		this._initializationPromise = this.queue(async () => {
			...
			if (this._deleted) return;
			if ((_a = this._popupRedirectResolver) === null || _a === void 0 ? void 0 : _a._shouldInitProactively) try {
	...
	*/
	...
	async initializeRecaptchaConfig() {
		const config = new RecaptchaConfig(await getRecaptchaConfig(this, {
			clientType: "CLIENT_TYPE_WEB",
			version: "RECAPTCHA_ENTERPRISE"
		}));
		if (this.tenantId == null) this._agentRecaptchaConfig = config;
		else this._tenantRecaptchaConfigs[this.tenantId] = config;
		if (config.emailPasswordEnabled) new RecaptchaEnterpriseVerifier(this).verify();
	}
	_getRecaptchaConfig() {
...
var EmailAuthCredential = class EmailAuthCredential extends AuthCredential {
	...
	*/
	...
	async _getIdTokenResponse(auth) {
		...
		switch (this.signInMethod) {
			case "password":
				...
				if ((_a = auth._getRecaptchaConfig()) === null || _a === void 0 ? void 0 : _a.emailPasswordEnabled) return signInWithPassword(auth, await injectRecaptchaFields(auth, request, "signInWithPassword"));
				else return signInWithPassword(auth, request).catch(async (error) => {
					if (error.code === `auth/missing-recaptcha-token`) {
						console.log("Sign-in with email address and password is protected by reCAPTCHA for this project. Automatically triggering the reCAPTCHA flow and restarting the sign-in flow.");
						return signInWithPassword(auth, await injectRecaptchaFields(auth, request, "signInWithPassword"));
	...
	async _linkToIdToken(auth, idToken) {
		switch (this.signInMethod) {
			case "password": return updateEmailPassword(auth, {
				idToken,
				returnSecureToken: true,
				email: this._email,
				password: this._password
...
async function signInAnonymously(auth) {
	...
	const response = await signUp(authInternal, { returnSecureToken: true });
...
async function createUserWithEmailAndPassword(auth, email, password) {
	...
	let signUpResponse;
	if ((_a = authInternal._getRecaptchaConfig()) === null || _a === void 0 ? void 0 : _a.emailPasswordEnabled) signUpResponse = signUp(authInternal, await injectRecaptchaFields(authInternal, request, "signUpPassword"));
	else signUpResponse = signUp(authInternal, request).catch(async (error) => {
		if (error.code === `auth/missing-recaptcha-token`) {
			console.log("Sign-up is protected by reCAPTCHA for this project. Automatically triggering the reCAPTCHA flow and restarting the sign-up flow.");
			return signUp(authInternal, await injectRecaptchaFields(authInternal, request, "signUpPassword"));
		} else return Promise.reject(error);
	});
...
function initializeRecaptchaConfig(auth) {
	return _castAuth(auth).initializeRecaptchaConfig();
}
/**
...
var BrowserLocalPersistence = class extends BrowserPersistenceClass {
	...
	onStorageEvent(event, poll = false) {
		...
		if (this.safariLocalStorageNotSynced) {
			...
			if (event.newValue !== storedValue) if (event.newValue !== null) this.storage.setItem(key, event.newValue);
			else this.storage.removeItem(key);
			else if (this.localCache[key] === event.newValue && !poll) return;
		}
...
var ReCaptchaLoaderImpl = class {
	...
	load(auth, hl = "") {
		...
		return new Promise((resolve, reject) => {
			...
			_loadJS(`${RECAPTCHA_BASE}?${querystring({
...
async function linkWithPhoneNumber(user, phoneNumber, appVerifier) {
	...
	return new ConfirmationResultImpl(await _verifyPhoneNumber(userInternal.auth, phoneNumber, getModularInstance(appVerifier)), (cred) => linkWithCredential(userInternal, cred));
}
/**
...
async function reauthenticateWithPhoneNumber(user, phoneNumber, appVerifier) {
	const userInternal = getModularInstance(user);
	return new ConfirmationResultImpl(await _verifyPhoneNumber(userInternal.auth, phoneNumber, getModularInstance(appVerifier)), (cred) => reauthenticateWithCredential(userInternal, cred));
}
/**
	...
	try {
		...
		if ("session" in phoneInfoOptions) {
			...
			if ("phoneNumber" in phoneInfoOptions) {
				...
				return (await startEnrollPhoneMfa(auth, {
				...
				return (await startSignInPhoneMfa(auth, {
